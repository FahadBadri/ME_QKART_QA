...
public class Home {
    ...
    public Boolean addProductToCart(String productName) {
        try {
            ...
            List<WebElement> titleElemets = driver.findElements(
                    By.xpath("//p[@class='MuiTypography-root MuiTypography-body1 css-yg30e6']"));
            List<WebElement> addtoCartElemets =
                    driver.findElements(By.xpath("//button[text()='Add to cart']"));

            for (int i = 0; i < titleElemets.size(); i++) {
                WebElement titlElement = titleElemets.get(i);
                String actualProduct = titlElement.getText();
                if (actualProduct.equals(productName)) {
                    WebElement addtocart = addtoCartElemets.get(i);
                    addtocart.click();
    ...
    public Boolean changeProductQuantityinCart(String productName, int quantity) {
        try {
            ...
            List<WebElement> titleOfElements =
            ...
            for (int i = 0; i < titleOfElements.size(); i++) {
                WebElement titleofElement = titleOfElements.get(i);
                String actualProductNAme = titleofElement.getText();
                if (actualProductNAme.equals(productName)) {
                    while (true) {
                        WebElement ActualProduct = ActualProducts.get(i);
                        String ActualQtText = ActualProduct.getText();
                        int Actualqtn = Integer.parseInt(ActualQtText);
                        if (quantity > Actualqtn) {
                            WebElement Plusbutton = PlusIcons.get(i);
                            Plusbutton.click();
                            Thread.sleep(1000);
    ...
    public Boolean verifyCartContents(List<String> expectedCartContents) {
        try {
            ...
            List<WebElement> actualsElements =
